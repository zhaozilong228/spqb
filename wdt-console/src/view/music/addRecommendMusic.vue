<template>
  <div>
      <Modal :title="recommendDate" v-model="showAddUpdModal"  @on-cancel="closeMusicModel" :mask-closable="false" :width="1200" :loading="true">
        <Card>
            <Row>
                <Col span="2">
...
export default {
    ...
    methods: {
        ...
        save() {
            ...
            this.$http.post('music/recommend/addRecommendtAttach', this.musicList).then(({ resultData }) => {
        ...
        initAddMusic(row) {
            this.disabledSave = true;
            this.addMusicParam = row;
            this.showAddUpdModal = true;
            this.$http.post('music/recommend/findRecommendList',{
                recommendDate: row.date
            }).then(({ resultData }) => {
               this.tabLoading = false
              console.log('返回的结果', resultData);
              this.isAddMap = {};
