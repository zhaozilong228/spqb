        ...
        <Card>
            <Row>
                <Col span="2">
                    ...
                    <Button type="primary" @click="openMusicModel">添加歌单</Button>
                </Col>
                <Col span="22">
...
import QueryMusic from './queryMusic'
export default {
    name: 'PlaylistManage',
    components: {
        AddMusic,
    ...
    methods: {
        ...
        save() {
          this.$refs['addUpdParamForm'].validate(valid => {
              if (valid) {
                ...
                const url = this.isUpdate ? 'music/play/updatePlayList' : 'music/play/addPlayList'
        ...
        searchPageList(){
           ...
           this.$http.post('music/play/findPlayPageList', this.queryParam).then(({ resultData }) => {
        ...
        updateStatus(row) {
          if (row.status && row.status === '101'){
            this.$http.post('music/play/statusOff/' + row.code).then(({ resultData }) => {
              this.$Modal.remove();
              this.searchPageList()
           }).catch((error) => {
             console.log('查询日志异常', error);
           ...
           this.$http.post('music/play/statusOn/' + row.code).then(({ resultData }) => {
            this.$Modal.remove();
            this.searchPageList()
          }).catch((error) => {
            console.log('查询日志异常', error);
        ...
        updateRecommend(row) {
          if (row.recommend && row.recommend === '101'){
            this.$http.post('music/play/recommendOff/' + row.code).then(({ resultData }) => {
              this.$Modal.remove();
              this.searchPageList()
           }).catch((error) => {
             console.log('查询日志异常', error);
           ...
           this.$http.post('music/play/recommendOn/' + row.code).then(({ resultData }) => {
            this.$Modal.remove();
            this.searchPageList()
          }).catch((error) => {
            console.log('查询日志异常', error);
        ...
        delplayList(row) {
           this.$http.post('music/play/delPlayList/' + row.code).then(({ resultData }) => {
             this.$Modal.remove();
             this.searchPageList()
          }).catch((error) => {
            console.log('查询日志异常', error);
