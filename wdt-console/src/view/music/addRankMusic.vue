  ...
  <div>
      <Modal :title="playListName" v-model="showAddUpdModal"  @on-cancel="closeMusicModel" :mask-closable="false" :width="1200" :loading="true">
        <Card>
            ...
            <Row>
                <Alert show-icon>点击复选框添加音乐，复选禁用时表示歌曲已存在当前榜单中，然后点击右下方添加</Alert>
...
export default {
    ...
    methods: {
        ...
        save() {
            ...
            if (this.selectionMusicList && this.selectionMusicList.length >0) {
                this.selectionMusicList.forEach( (item) =>{
                    ...
                    if (!isExist) {
                        this.musicList.push({
                            ranklistName: this.addMusicParam.name,
                            ranklistCode : this.addMusicParam.code,
                            musicName: item.name,
                            musicCode: item.code,
            ...
            this.$http.post('music/rank/addRankListAttach', this.musicList).then(({ resultData }) => {
        ...
        initAddMusic(param) {
            ...
            this.showAddUpdModal = true;
            this.$http.post('music/rank/findRankByCode/' + param.row.code).then(({ resultData }) => {
               this.tabLoading = false
              console.log('返回的结果', resultData);
              this.isAddMap = {};
